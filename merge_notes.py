#!/usr/bin/env python3
"""Merge lecture slides into markdown + PDF notes (2 slides per page)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Output base name (writes output/notes/<name>.md and .pdf)",
    )
    return parser.parse_args()


def resolve_paths(args: argparse.Namespace) -> Paths:
    out_dir = ROOT / "output/notes"
    stem = args.name
    return Paths(
...
def slide_body(lines: list[str], title: str) -> str:
    body: list[str] = []
    for line in lines:
        if line == title:
            continue
        low = line.lower().strip()
        if low in {"brainstorm", "your turn!", "your turn", "yes", "no", "??"}:
            continue
        if re.match(r"^(yes|no|\?\?|ip \?|uniqueness \?)$", low):
            continue
...
def extract_thumb(page_index: int, page: fitz.Page, img_dir: Path) -> Path:
    thumb_path = img_dir / f"slide-{page_index + 1:02d}-thumb.jpg"
    zoom = 2.2
...
def collect_slides(paths: Paths) -> list[SlideNote]:
    ...
    for i in range(doc.page_count):
        ...
        body = slide_body(lines, title)
        thumb = extract_thumb(i, page, paths.img_dir)
        spoken = TRANSCRIPT_BY_SLIDE[i] if i < len(TRANSCRIPT_BY_SLIDE) else ""
        slides.append(SlideNote(index=i + 1, title=title, body=body, thumb=thumb, spoken=spoken))

    doc.close()
    return slides


def build_markdown(slides: list[SlideNote], paths: Paths) -> None:
    lines_out: list[str] = [
        f"# {COVER['title']} — {COVER['subtitle']}",
        "",
        "**Source slides:** slides PDF  ",
    ...
    for slide in slides:
        lines_out.append(f"## {slide.title}")
        lines_out.append("")
        lines_out.append(
            f'<img src="images/{slide.thumb.name}" width="640" alt="{slide.title}" />'
        )
        lines_out.append("")

        if slide.body:
            lines_out.append(slide.body)
            lines_out.append("")

        if slide.spoken:
            lines_out.append(f"*{slide.spoken}*")
            lines_out.append("")
...
def build_notes(paths: Paths) -> tuple[Path, Path]:
    slides = collect_slides(paths)
    build_markdown(slides, paths)
    build_pdf(slides, paths)
    return paths.out_md, paths.out_pdf


if __name__ == "__main__":
    args = parse_args()
    paths = resolve_paths(args)
    md_path, pdf_path = build_notes(paths)
